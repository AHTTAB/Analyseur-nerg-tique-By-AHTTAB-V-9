...
import { ArrowLeft, Upload, AlertCircle, Activity, TrendingUp, Clock, Calendar, Plus, Minus, Files, Printer } from 'lucide-react';
import { Button } from '../components/Button';
import { Card } from '../components/Card';
import { useAuth } from '../context/AuthContext';
import { ViewState, AnalysisResult } from '../types';
import { processElsterFiles } from '../utils/excelParser';
import { exportToExcel } from '../utils/excelExporter';

interface ElsterAnalysisProps {
  onNavigate: (view: ViewState) => void;
}

export const ElsterAnalysis: React.FC<ElsterAnalysisProps> = ({ onNavigate }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const data = await processElsterFiles(files);
      setResults(data);
      ...
      if (user) {
        ...
        ], `Analyse_Elster_${user.username}_${new Date().getTime()}`);
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 print:py-0 print:px-0">
      ...
      <div className="hidden print:block text-center mb-8 pt-4">
        ...
        <p className="text-sm text-slate-500">Rapport d'analyse Compteur Elster - {new Date().toLocaleDateString()}</p>
      </div>
        ...
        <Card className="max-w-xl mx-auto text-center py-12 print:hidden">
          <div className="mb-6 flex justify-center">
            <div className="bg-indigo-50 p-4 rounded-full">
              {selectedFiles.length > 0 ? <Files className="w-12 h-12 text-indigo-500" /> : <Upload className="w-12 h-12 text-indigo-500" />}
            </div>
          </div>
          <h3 className="text-xl font-semibold mb-2">Importer des fichiers Excel</h3>
          <p className="text-slate-500 mb-8 max-w-sm mx-auto">
            Sélectionnez 1 à 4 fichiers. Les données de la 2ème feuille (P. Active) seront additionnées.
          </p>
          ...
          <div className="relative inline-block">
             <input
              ...
              disabled={loading}
            />
            <Button variant="primary" disabled={loading} className="pointer-events-none bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500">
      ...
      {results && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <Card title={`Top 5 - Valeurs les plus élevées (${selectedFiles.length > 1 ? 'Global / Somme' : 'Elster'})`} className="print:shadow-none print:border print:border-slate-300">
                ...
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50 print:bg-slate-100">
                    ...
                    {results.headers.map((header, idx) => (
                         <th key={idx} className={`py-3 px-4 font-semibold text-slate-600 ${idx === results.headers.length - 2 ? 'text-indigo-700 bg-indigo-50/50 ring-1 ring-indigo-100 print:bg-transparent print:ring-0 print:text-slate-800' : ''}`}>
                             {header || `Col ${idx + 1}`}
                         </th>
                    ))}
                ...
                <tbody>
                  {results.topFiveGlobal.map((row, idx) => (
                    <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 print:break-inside-avoid">
                      ...
                      {results.headers.map((_, colIdx) => {
                          const isTargetCol = colIdx === results.headers.length - 2;
                          return (
                            <td key={colIdx} className={`py-3 px-4 text-slate-700 ${isTargetCol ? 'font-bold text-indigo-600 bg-indigo-50/10 print:bg-transparent print:text-black' : ''}`}>
                                {renderCell(row.fullRow[colIdx])}
                            </td>
                          );
                      })}
