  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 print:py-0 print:px-0">
      ...
      <div className="hidden print:block text-center mb-8 pt-4">
        ...
        <p className="text-sm text-slate-500">Rapport d'analyse Compteur Clou - {new Date().toLocaleDateString()}</p>
      </div>
        ...
        <Card className="max-w-xl mx-auto text-center py-12 print:hidden">
          <div className="mb-6 flex justify-center">
            <div className="bg-blue-50 p-4 rounded-full">
              {selectedFiles.length > 0 ? <Files className="w-12 h-12 text-blue-500" /> : <Upload className="w-12 h-12 text-blue-500" />}
            </div>
          </div>
          <h3 className="text-xl font-semibold mb-2">Importer des fichiers Excel</h3>
          <p className="text-slate-500 mb-8 max-w-sm mx-auto">
            Sélectionnez 1 à 4 fichiers contenant la feuille "Profil de charge moyen". Les valeurs P. Active seront additionnées.
          </p>
          ...
          <div className="relative inline-block">
             <input
              ...
              disabled={loading}
            />
            <Button variant="primary" disabled={loading} className="pointer-events-none">
      ...
      {results && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <Card title={`Top 5 - Valeurs les plus élevées (${selectedFiles.length > 1 ? 'Global / Somme' : 'Détails'})`} className="print:shadow-none print:border print:border-slate-300">
                ...
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50 print:bg-slate-100">
                    ...
                    {results.headers.map((header, idx) => (
                         <th key={idx} className={`py-3 px-4 font-semibold text-slate-600 ${idx === results.headers.length - 2 ? 'text-blue-700 bg-blue-50/50 ring-1 ring-blue-100 print:bg-transparent print:ring-0 print:text-slate-800' : ''}`}>
                             {header || `Col ${idx + 1}`}
                         </th>
                    ))}
                  ...
                  {results.topFiveGlobal.map((row, idx) => (
                    <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 print:break-inside-avoid">
                      ...
                      {results.headers.map((_, colIdx) => {
                          ...
                          return (
                            <td key={colIdx} className={`py-3 px-4 text-slate-700 ${isTargetCol ? 'font-bold text-blue-600 bg-blue-50/10 print:bg-transparent print:text-black' : ''}`}>
          ...
          <div className="grid md:grid-cols-3 gap-6 print:grid-cols-3 print:gap-4">
            {[results.heuresPleines, results.heuresPointe, results.heuresCreuses].map((period, idx) => {
               ...
               if (idx === 1) { icon = <TrendingUp className="w-6 h-6" />; bgClass = "bg-rose-500"; }
               if (idx === 2) { icon = <Calendar className="w-6 h-6" />; bgClass = "bg-emerald-500"; }
                ...
                <Card key={idx} className="relative overflow-hidden print:shadow-none print:border print:border-slate-300 print:break-inside-avoid">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className={`text-3xl font-bold ${idx === 1 ? 'text-rose-600' : (idx === 2 ? 'text-emerald-600' : 'text-blue-600')} print:text-black`}>
