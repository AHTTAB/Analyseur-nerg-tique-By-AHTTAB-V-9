...
import { ArrowLeft, Upload, AlertCircle, Zap, TrendingUp, Clock, Calendar, Plus, Minus, Files, Printer } from 'lucide-react';
import { Button } from '../components/Button';
import { Card } from '../components/Card';
import { useAuth } from '../context/AuthContext';
import { ViewState, AnalysisResult } from '../types';
import { processActarisFiles } from '../utils/excelParser';
import { exportToExcel } from '../utils/excelExporter';

interface ActarisAnalysisProps {
  onNavigate: (view: ViewState) => void;
}

export const ActarisAnalysis: React.FC<ActarisAnalysisProps> = ({ onNavigate }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const data = await processActarisFiles(files);
      setResults(data);
      ...
      if (user) {
        ...
        ], `Analyse_Actaris_${user.username}_${new Date().getTime()}`);
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 print:py-0 print:px-0">
      ...
      <div className="hidden print:block text-center mb-8 pt-4">
        ...
        <p className="text-sm text-slate-500">Rapport d'analyse Compteur Actaris - {new Date().toLocaleDateString()}</p>
      </div>
        ...
        <Card className="max-w-xl mx-auto text-center py-12 print:hidden">
          <div className="mb-6 flex justify-center">
            <div className="bg-amber-50 p-4 rounded-full">
              {selectedFiles.length > 0 ? <Files className="w-12 h-12 text-amber-500" /> : <Upload className="w-12 h-12 text-amber-500" />}
            </div>
          </div>
          <h3 className="text-xl font-semibold mb-2">Importer des fichiers Excel</h3>
          <p className="text-slate-500 mb-8 max-w-sm mx-auto">
            Sélectionnez 1 à 4 fichiers. Le système cherchera la feuille "Données des Courbes de Charge".
          </p>
          ...
          <div className="relative inline-block">
             <input
              ...
              disabled={loading}
            />
            <Button variant="primary" disabled={loading} className="pointer-events-none bg-amber-600 hover:bg-amber-700 focus:ring-amber-500">
      ...
      {results && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <Card title={`Top 5 - Valeurs les plus élevées (${selectedFiles.length > 1 ? 'Global / Somme' : 'Actaris'})`} className="print:shadow-none print:border print:border-slate-300">
                ...
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/50 print:bg-slate-100">
                    ...
                    {results.headers.map((header, idx) => (
                         <th key={idx} className={`py-3 px-4 font-semibold text-slate-600 ${header.includes('Active') ? 'text-amber-700 bg-amber-50/50 ring-1 ring-amber-100 print:bg-transparent print:ring-0 print:text-slate-800' : ''}`}>
                             {header || `Col ${idx + 1}`}
                         </th>
                    ))}
                ...
                <tbody>
                  {results.topFiveGlobal.map((row, idx) => (
                    <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 print:break-inside-avoid">
                      ...
                      {results.headers.map((header, colIdx) => {
                          const isTargetCol = header.includes('Active');
                          return (
                            <td key={colIdx} className={`py-3 px-4 text-slate-700 ${isTargetCol ? 'font-bold text-amber-600 bg-amber-50/10 print:bg-transparent print:text-black' : ''}`}>
